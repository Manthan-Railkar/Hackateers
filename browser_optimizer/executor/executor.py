...
class RuleBasedExecutor:
    ...
    async def execute(
        ...
        # Capture live visual snapshot for Mission Control UI
        try:
            from browser_optimizer.browser.manager import live_screenshot_store
            await live_screenshot_store.capture(page, session_id, f"{action} {selector or ''} {value or ''}".strip())
        except Exception:
            pass
